...
import { useCorrectiveStatuses } from '../hooks/use-corrective-statuses';
import { useCorrectiveStatusOptions } from '../hooks/use-corrective-status-options';
import { useCorrectiveStatusFilters } from '../hooks/use-corrective-status-filters';
...
export function CorrectiveStatusList() {
  const { data: statuses = [], isLoading, error } = useCorrectiveStatuses();
  const { data: displayStatusOptions = [] } = useCorrectiveStatusOptions();
  const {
    searchTerm,
    setSearchTerm,
    statusFilter,
    ...
    setSortBy,
    filteredStatuses,
  } = useCorrectiveStatusFilters(statuses);
  const pagination = usePagination(filteredStatuses, { initialItemsPerPage: 10 });
  const { resetPage } = pagination;

  const createMutation = useCreateCorrectiveStatus();
  const updateMutation = useUpdateCorrectiveStatus();
  const deleteMutation = useDeleteCorrectiveStatus();
          ...
          <form onSubmit={handleCreate} className="space-y-4">
            <div>
              ...
              <Input
                ...
                placeholder="e.g., En Cola de Desarrollo"
                className="w-full"
              />
