...
import { useUploadProblems } from '../hooks/use-upload-problems';
import { useDeleteProblems } from '../hooks/use-delete-problems';
import { useProblemFilters } from '../hooks/use-problem-filters';

export function ProblemsList() {
  const { data, isLoading, refetch } = useProblems();
  const uploadMutation = useUploadProblems();
  const deleteMutation = useDeleteProblems();
  ...
  }, [searchTerm, applicationFilter, categoryFilter]);
  ...
  useEffect(() => {
    const saved = localStorage.getItem('lastUploadedProblemsFile');
  ...
  const handleDeleteAll = () => {
    deleteMutation.mutate(undefined, {
      onSuccess: (result) => {
        toast.success(result.message || 'All problems deleted successfully');
  ...
  const withActionPlans = problems.filter(
    (p) => p.planesDeAccion !== 'No asignado'
  ).length;
  ...
  const statsData = [
    ...
    {
      label: 'With Action Plans',
      value: withActionPlans,
    ...
    <>
      ...
      {statsData.length > 0 && (
        <StatsGrid
          stats={statsData}
          onRefresh={() => refetch()}
          onClearData={problems.length > 0 ? () => setShowDeleteDialog(true) : undefined}
          loading={loading}
        />
      )}
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete all data from the problems table. This
