...
export default function MonthlyReportStatusRegistryPage() {
  ...
  const fetchStatuses = async () => {
    setLoading(true);
    try {
      const response = await fetch('http://localhost:3000/monthly-report-status-registry', {
        cache: 'no-store',
      });
      if (response.ok) {
  ...
  // Open edit modal
  const openEditModal = (status: RequestStatus) => {
  ...
  const openDeleteConfirm = (status: RequestStatus) => {
    setSelectedStatus(status);
            ...
            <form onSubmit={handleCreateStatus} className="space-y-4">
              ...
              <div>
                ...
                >
                  ...
                  <option value="On going L2">On going L2</option>
                  <option value="On going L3">On going L3</option>
                  <option value="In L3 Backlog">In L3 Backlog</option>
                </select>
                <p className="text-xs text-muted-foreground/60 mt-1">
