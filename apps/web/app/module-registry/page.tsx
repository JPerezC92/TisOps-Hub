...
const APPLICATIONS = ['CD', 'FFVV', 'SB', 'UNETE'] as const;

const APPLICATION_COLORS: Record<string, { bg: string; text: string; border: string }> = {
...
export default function ModuleRegistryPage() {
  ...
  const fetchModules = async () => {
    setLoading(true);
    try {
      const response = await fetch('http://localhost:3000/module-registry', {
        cache: 'no-store',
      });
      if (response.ok) {
        ...
        alert('Failed to fetch module mappings');
      }
    } catch (error) {
  ...
  const handleCreateModule = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({ sourceValue: '', displayValue: '', application: 'SB' });
      ...
      alert('Failed to create module mapping');
    }
  };
  ...
  const handleUpdateModule = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`http://localhost:3000/module-registry/${selectedModule.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
      ...
      alert('Failed to update module mapping');
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      alert('Failed to delete module mapping');
    }
  };
  ...
  const openEditModal = (module: Module) => {
    ...
    setFormData({
      ...
      displayValue: module.displayValue,
  ...
  const filteredModules = modules
    .filter((mod) => {
      ...
      const matchesStatus =
        statusFilter === 'all' || (statusFilter === 'active' ? mod.isActive : !mod.isActive);
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, statusFilter, applicationFilter, sortBy]);

  const getApplicationColors = (app: string) => {
  ...
  return (
    <div className="min-h-screen bg-background">
      <main className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <p className="mt-3 text-base text-muted-foreground/90">
            Manage module mappings (Source Value → Display Value) with application indicators
          </p>
        </div>
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 backdrop-blur-sm">
          <div className="bg-card border border-jpc-vibrant-cyan-500/30 rounded-2xl p-8 max-w-md w-full mx-4 shadow-2xl">
            <h2 className="text-2xl font-bold text-foreground mb-6">Create New Module Mapping</h2>
      ...
      {showEditModal && selectedModule && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 backdrop-blur-sm">
          <div className="bg-card border border-jpc-vibrant-cyan-500/30 rounded-2xl p-8 max-w-md w-full mx-4 shadow-2xl">
            <h2 className="text-2xl font-bold text-foreground mb-6">Edit Module Mapping</h2>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 backdrop-blur-sm">
          <div className="bg-card border border-red-500/30 rounded-2xl p-8 max-w-md w-full mx-4 shadow-2xl">
            ...
            <p className="text-sm text-muted-foreground mb-6">
              Are you sure you want to delete the mapping for &quot;{selectedModule.sourceValue}&quot;? This action cannot
              be undone.
            </p>
